...
normalize gs
	| gsPlayerIndex gs >= gsN gs + 1 = error "index too big"
	| gsPlayerIndex gs <= -2 = error "index too small"

	| gsSweep gs == SweepLeft &&
		gsPlayerIndex gs == gsN gs =
			gs { gsN = gsN gs - 1, gsSweep = SweepRight }
	| gsSweep gs == SweepRight &&
		gsPlayerIndex gs == (-1) =
			gs {gsN = 0, gsSweep = SweepLeft }
...
children gs = [ normalize (pick i) | i <- [0 .. length (gsInventory gs) - 1], not (elem i (gsIgnoreFirst gs)) ] where
	pick i = 
		let picked = (gsInventory gs) !! i in
		gs {
			gsN = gsN gs - 1,
